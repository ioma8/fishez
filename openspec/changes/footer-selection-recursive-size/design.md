...
**Footer placeholder: reuse the existing count-based text.** While `dir_total` or `selection_total` is `Computing`/`Idle`, `draw_footer`'s multi-select branch shows `"Selected: {count}"` (today's text) unchanged; once both are `Ready`, it switches to `"Selected: {size} of {total}"`. No new placeholder string to design or test separately.
